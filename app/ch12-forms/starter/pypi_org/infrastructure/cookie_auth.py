...
def get_user_id_via_auth_cookie(request: Request) -> Optional[int]:
    if auth_cooke_name not in request.cookies:
        return None

    val = request.cookies[auth_cooke_name]
    parts = val.split(':')
    if len(parts) != 2:
        return None
...
def logout(response: Response):
    response.delete_cookie(auth_cooke_name)